...
gencode  *final;                          /* output, code for the final hash */
{
  ...
  else
  {
    ...
    final->used = 2;
    sprintf(final->line[0], 
	    "  unsigned long rsl, val = lookup(key, len, 0x%lx);\n", initlev);
    if (smax <= 1)
    {
...
ub4  salt;
{
  ...
  if (blen > 0)
  {
    ...
    else
    {
      ...
      {
	if (smax <= UB2MAXVAL+1)
	  fprintf(f, "extern unsigned short scramble[];\n");
	else
	  fprintf(f, "extern unsigned long  scramble[];\n");
      }
    }
